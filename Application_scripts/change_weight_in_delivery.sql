...
DROP PROCEDURE IF EXISTS `change_weight_in_delivery`//
CREATE DEFINER=`root`@`localhost` PROCEDURE `change_weight_in_delivery`(IN `changed_good_ID` INT(11), IN `addition` INT(11))

BEGIN

UPDATE delivery SET delivery.delivery_total_weight = delivery.delivery_total_weight + addition
